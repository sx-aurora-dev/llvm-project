...
void VE::writePltHeader(uint8_t *buf) const {
  if (ctx.arg.isPic) {
    // PIC PLT header: use %s15 (GOT pointer, set by dynamic linker).
    const uint8_t pltData[] = {
                                                        // _PROCEDURE_LINKAGE_TABLE:
        0x00, 0x00, 0x00, 0x00, 0x8f, 0x00, 0x3e, 0x45, // or %s62, 0, %s15
        0x08, 0x00, 0x00, 0x00, 0x8f, 0x00, 0x3f, 0x01, // ld %s63, 8(, %s15)
    ...
    // Non-PIC PLT header: use absolute GOT address.
    const uint8_t pltData[] = {
                                                        // _PROCEDURE_LINKAGE_TABLE:
...
void VE::writePlt(uint8_t *buf, const Symbol & sym,
  ...
  if (ctx.arg.isPic) {
    // PIC PLT entry: use %s15 (GOT pointer) + offset to access GOT.PLT.
    const uint8_t pltData[] = {
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0d, 0x06, // lea %s13, offset_from_gotplt_base
        0x00, 0x00, 0x00, 0x00, 0x60, 0x8d, 0x0d, 0x44, // and %s13, %s13, (32)0
        0x00, 0x00, 0x00, 0x00, 0x8d, 0x8f, 0x8d, 0x06, // lea.sl %s13, (%s15, %s13)
        0x00, 0x00, 0x00, 0x00, 0x8d, 0x00, 0x0c, 0x01, // ld %s12, (, %s13)
    ...
    uint64_t offset = gotPlt + pltEntryIdx * gotEntrySize - gotPltBase;
    // Set offset from GOT.PLT base to the entry
    relocateNoSym(buf + 0 * 8, R_VE_LO32, offset);
  } else {
    // Non-PIC PLT entry: use absolute GOT.PLT address.
    const uint8_t pltData[] = {
