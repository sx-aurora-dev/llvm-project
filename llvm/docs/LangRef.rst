...
-----------------------------
VP intrinics are intended for predicated SIMD/vector code.  A typical VP
...
The vector mask parameter always has a vector of `i1` type, for example `<32 x
i1>`.  The explicit vector length parameter always has the type `i32` and is a
signed integer value.  The explicit vector length is only effective if it is
non-negative.  Results are only computed for enabled lanes.  A lane is enabled
if the mask at that position is true and, if effective, where the lane position
is below the explicit vector length.
