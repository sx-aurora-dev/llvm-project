...
static SDValue widenVectorToPartType(SelectionDAG &DAG, SDValue Val,
  ...
  // Widening a fixed-length vector to another fixed-length vector.
  // NOTE: Original comment said "Widening a scalable vector to another scalable
  // vector is done by inserting the vector into a larger undef one." but this
  // block actually handles fixed-length vectors. Kept for future review.
