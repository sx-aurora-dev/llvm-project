...
int TargetTransformInfo::getIntrinsicInstrCost(Intrinsic::ID ID, Type *RetTy,
                                               ...
                                               TTI::TargetCostKind CostKind,
                                               const Instruction *I) const {
  int Cost = TTIImpl->getIntrinsicInstrCost(
      ID, RetTy, Tys, FMF, ScalarizationCostPassed, CostKind, I);
  assert(Cost >= 0 && "TTI should not produce negative costs!");
  return Cost;
}
...
int TargetTransformInfo::getInstructionThroughput(const Instruction *I) const {
  ...
  case Instruction::Xor: {
    ...
    return getArithmeticInstrCost(I->getOpcode(), I->getType(), CostKind, Op1VK,
                                  Op2VK, Op1VP, Op2VP, Operands, I);
  }
  ...
  case Instruction::Select: {
    ...
    return getCmpSelInstrCost(I->getOpcode(), I->getType(), CondTy, CostKind,
                              I);
  }
  case Instruction::ICmp:
  case Instruction::FCmp: {
    Type *ValTy = I->getOperand(0)->getType();
    return getCmpSelInstrCost(I->getOpcode(), ValTy, I->getType(), CostKind, I);
  }
