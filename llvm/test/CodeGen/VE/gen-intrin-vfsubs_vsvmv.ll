...
; ModuleID = 'gen/tests/vfsubs_vsvmv.c'
source_filename = "gen/tests/vfsubs_vsvmv.c"
target datalayout = "e-m:e-i64:64-n32:64-S64-v16384:64:64"
target triple = "ve"

; Function Attrs: nounwind
define dso_local void @vfsubs_vsvmv(float* %pvx, float %sy, float* %pvz, i32* %pvm, float* nocapture readnone %pvd, i32 %n) local_unnamed_addr #0 {
; CHECK-LABEL: vfsubs_vsvmv
; CHECK: .LBB0_2
; CHECK: 	vfsub.s %v2,%s1,%v0,%vm1
entry:
...
for.body:                                         ; preds = %entry, %for.body
  %pvx.addr.025 = phi float* [ %add.ptr, %for.body ], [ %pvx, %entry ]
  %pvz.addr.024 = phi float* [ %add.ptr3, %for.body ], [ %pvz, %entry ]
  ...
  %0 = bitcast float* %pvz.addr.024 to i8*
  ...
  %5 = bitcast float* %pvx.addr.025 to i8*
  %6 = tail call <256 x double> @llvm.ve.vldu.vss(i64 4, i8* %5)
  %7 = tail call <256 x double> @llvm.ve.vfsubs.vsvmv(float %sy, <256 x double> %1, <4 x i64> %4, <256 x double> %6)
  tail call void @llvm.ve.vstu.vss(<256 x double> %7, i64 4, i8* %5)
  %add.ptr = getelementptr inbounds float, float* %pvx.addr.025, i64 256
  %add.ptr3 = getelementptr inbounds float, float* %pvz.addr.024, i64 256
...
; Function Attrs: nounwind readnone
declare <256 x double> @llvm.ve.vfsubs.vsvmv(float, <256 x double>, <4 x i64>, <256 x double>) #3
