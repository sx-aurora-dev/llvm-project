...
; ModuleID = 'gen/tests/vfnmads_vsvvmv.c'
source_filename = "gen/tests/vfnmads_vsvvmv.c"
target datalayout = "e-m:e-i64:64-n32:64-S64-v16384:64:64"
target triple = "ve"

; Function Attrs: nounwind
define dso_local void @vfnmads_vsvvmv(float* %pvx, float %sy, float* %pvz, float* %pvw, i32* %pvm, float* nocapture readnone %pvd, i32 %n) local_unnamed_addr #0 {
; CHECK-LABEL: vfnmads_vsvvmv
; CHECK: .LBB0_2
; CHECK: 	vfnmad.s %v3,%s1,%v0,%v1,%vm1
entry:
...
for.body:                                         ; preds = %entry, %for.body
  ...
  %pvz.addr.028 = phi float* [ %add.ptr3, %for.body ], [ %pvz, %entry ]
  ...
  %0 = bitcast float* %pvz.addr.028 to i8*
  ...
  %9 = tail call <256 x double> @llvm.ve.vfnmads.vsvvmv(float %sy, <256 x double> %1, <256 x double> %3, <4 x i64> %6, <256 x double> %8)
  tail call void @llvm.ve.vstu.vss(<256 x double> %9, i64 4, i8* %7)
  %add.ptr = getelementptr inbounds float, float* %pvx.addr.029, i64 256
  %add.ptr3 = getelementptr inbounds float, float* %pvz.addr.028, i64 256
...
; Function Attrs: nounwind readnone
declare <256 x double> @llvm.ve.vfnmads.vsvvmv(float, <256 x double>, <256 x double>, <4 x i64>, <256 x double>) #3
