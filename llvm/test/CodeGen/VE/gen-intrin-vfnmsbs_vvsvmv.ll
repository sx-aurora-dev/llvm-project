...
; ModuleID = 'gen/tests/vfnmsbs_vvsvmv.c'
source_filename = "gen/tests/vfnmsbs_vvsvmv.c"
target datalayout = "e-m:e-i64:64-n32:64-S64-v16384:64:64"
target triple = "ve"

; Function Attrs: nounwind
define dso_local void @vfnmsbs_vvsvmv(float* %pvx, float* %pvy, float %sy, float* %pvw, i32* %pvm, float* nocapture readnone %pvd, i32 %n) local_unnamed_addr #0 {
; CHECK-LABEL: vfnmsbs_vvsvmv
; CHECK: .LBB0_2
; CHECK: 	vfnmsb.s %v3,%v0,%s2,%v1,%vm1
entry:
...
for.body:                                         ; preds = %entry, %for.body
  ...
  %pvw.addr.027 = phi float* [ %add.ptr4, %for.body ], [ %pvw, %entry ]
  ...
  %2 = bitcast float* %pvw.addr.027 to i8*
  ...
  %9 = tail call <256 x double> @llvm.ve.vfnmsbs.vvsvmv(<256 x double> %1, float %sy, <256 x double> %3, <4 x i64> %6, <256 x double> %8)
  ...
  %add.ptr4 = getelementptr inbounds float, float* %pvw.addr.027, i64 256
...
; Function Attrs: nounwind readnone
declare <256 x double> @llvm.ve.vfnmsbs.vvsvmv(<256 x double>, float, <256 x double>, <4 x i64>, <256 x double>) #3
