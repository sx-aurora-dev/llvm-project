...
; ModuleID = 'gen/tests/vfnmsbd_vsvvmv.c'
source_filename = "gen/tests/vfnmsbd_vsvvmv.c"
target datalayout = "e-m:e-i64:64-n32:64-S64-v16384:64:64"
target triple = "ve"

; Function Attrs: nounwind
define dso_local void @vfnmsbd_vsvvmv(double* %pvx, double %sy, double* %pvz, double* %pvw, i32* %pvm, double* nocapture readnone %pvd, i32 %n) local_unnamed_addr #0 {
; CHECK-LABEL: vfnmsbd_vsvvmv
; CHECK: .LBB0_2
; CHECK: 	vfnmsb.d %v3,%s1,%v0,%v1,%vm1
entry:
...
for.body:                                         ; preds = %entry, %for.body
  ...
  %pvz.addr.028 = phi double* [ %add.ptr3, %for.body ], [ %pvz, %entry ]
  %pvw.addr.027 = phi double* [ %add.ptr4, %for.body ], [ %pvw, %entry ]
  ...
  %0 = bitcast double* %pvz.addr.028 to i8*
  %1 = tail call <256 x double> @llvm.ve.vld.vss(i64 8, i8* %0)
  %2 = bitcast double* %pvw.addr.027 to i8*
  ...
  %9 = tail call <256 x double> @llvm.ve.vfnmsbd.vsvvmv(double %sy, <256 x double> %1, <256 x double> %3, <4 x i64> %6, <256 x double> %8)
  ...
  %add.ptr3 = getelementptr inbounds double, double* %pvz.addr.028, i64 256
  %add.ptr4 = getelementptr inbounds double, double* %pvw.addr.027, i64 256
...
; Function Attrs: nounwind readnone
declare <256 x double> @llvm.ve.vfnmsbd.vsvvmv(double, <256 x double>, <256 x double>, <4 x i64>, <256 x double>) #3
