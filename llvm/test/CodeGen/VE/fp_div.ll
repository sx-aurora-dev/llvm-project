...
; CHECK-NEXT:    bsic %lr, (,%s12)
; CHECK-NEXT:    or %s11, 0, %s9
  %r = fdiv fp128 %x, 0xL00000000000000004001400000000000
  ret fp128 %r
}
...
; CHECK-NEXT:    fdiv.s %s0, %s0, %s1
; CHECK-NEXT:    or %s11, 0, %s9
  %r = fdiv float %x, 0x47EFFFFFE0000000
  ret float %r
}
...
; CHECK-NEXT:    fdiv.d %s0, %s0, %s1
; CHECK-NEXT:    or %s11, 0, %s9
  %r = fdiv double %x, 0x7FEFFFFFFFFFFFFF
  ret double %r
}
...
; CHECK-NEXT:    and %s2, %s2, (32)0
; CHECK-NEXT:    lea.sl %s4, .LCPI8_0@hi(%s2)
; CHECK-NEXT:    ld %s2, 8(,%s4)
; CHECK-NEXT:    ld %s3, (,%s4)
; CHECK-NEXT:    lea %s4, __divtf3@lo
; CHECK-NEXT:    and %s4, %s4, (32)0
...
; CHECK-NEXT:    bsic %lr, (,%s12)
; CHECK-NEXT:    or %s11, 0, %s9
  %r = fdiv fp128 %x, 0xLFFFFFFFFFFFFFFFF7FFEFFFFFFFFFFFF
  ret fp128 %r
}
