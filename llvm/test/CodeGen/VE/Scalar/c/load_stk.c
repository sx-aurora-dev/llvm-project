...
__attribute__ ((REGCALL)) \
TYPE KIND ## TYPE ## _stk_dyn(i64 bytes) { \
  volatile TYPE stk; \
...
__attribute__ ((REGCALL)) \
TYPE KIND ## TYPE ## _stk_dyn_align(i64 bytes) { \
...
__attribute__ ((REGCALL)) \
TYPE KIND ## TYPE ## _stk_dyn_align2(i64 bytes) { \
  volatile struct { TYPE stk; } s __attribute__ ((aligned (32))); \
  volatile TYPE* ptr = __builtin_alloca_with_align(bytes, 32); \
  TYPE v, v2, v3; \
  v2 = *ptr; \
...
__attribute__ ((REGCALL)) \
TYPE KIND ## TYPE ## _stk_dyn_align_spill(i64 bytes) { \
  volatile struct { TYPE stk; } s __attribute__ ((aligned (32))); \
  volatile TYPE* ptr = __builtin_alloca_with_align(bytes, 32); \
  extern void dummy(); \
