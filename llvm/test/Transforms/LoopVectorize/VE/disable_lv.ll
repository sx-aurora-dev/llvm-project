...
; RUN: opt < %s -rv=0  -loop-vectorize -ve-expensive-vector=1 -mtriple=ve-linux -S | FileCheck %s -check-prefix=SCALAR
; RUN: opt < %s -rv=0  -loop-vectorize -ve-expensive-vector=0 -mtriple=ve-linux -S | FileCheck %s -check-prefix=VECTOR
