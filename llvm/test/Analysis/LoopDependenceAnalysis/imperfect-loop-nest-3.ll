...
entry:
  ...
  br i1 %cmp9, label %for.body, label %for.end53

for.body:                                         ; preds = %entry, %for.inc51
  %k.010 = phi i64 [ %inc52, %for.inc51 ], [ 0, %entry ]
...
for.end22:                                        ; preds = %for.inc20, %for.body
  ...
  br i1 %cmp257, label %for.body26, label %for.inc51

for.body26:                                       ; preds = %for.end22, %for.inc48
  %i23.08 = phi i64 [ %inc49, %for.inc48 ], [ 0, %for.end22 ]
  %cmp295 = icmp slt i64 0, %z
  br i1 %cmp295, label %for.body30, label %for.inc48

for.body30:                                       ; preds = %for.body26, %for.body30
  %j27.06 = phi i64 [ %inc46, %for.body30 ], [ 0, %for.body26 ]
  %mul31 = mul nsw i64 %j27.06, %y
  %mul32 = mul nsw i64 %mul31, %z
  %mul33 = mul nsw i64 %k.010, %y
  %add34 = add nsw i64 %mul32, %mul33
  %add35 = add nsw i64 %add34, 0
  %arrayidx36 = getelementptr inbounds i64, i64* %A, i64 %add35
  %2 = load i64, i64* %arrayidx36, align 8
  %sub37 = sub nsw i64 %j27.06, 1
  %mul38 = mul nsw i64 %sub37, %y
  %mul39 = mul nsw i64 %mul38, %z
  %add40 = add nsw i64 %k.010, 4
  %mul41 = mul nsw i64 %add40, %y
  %add42 = add nsw i64 %mul39, %mul41
  %add43 = add nsw i64 %add42, 0
  %arrayidx44 = getelementptr inbounds i64, i64* %A, i64 %add43
  store i64 %2, i64* %arrayidx44, align 8
  %inc46 = add nsw i64 %j27.06, 1
  %cmp29 = icmp slt i64 %inc46, %z
  br i1 %cmp29, label %for.body30, label %for.inc48

for.inc48:                                        ; preds = %for.body26, %for.body30
  %inc49 = add nsw i64 %i23.08, 1
  %cmp25 = icmp slt i64 %inc49, %y
  br i1 %cmp25, label %for.body26, label %for.inc51

for.inc51:                                        ; preds = %for.end22, %for.inc48
  %inc52 = add nsw i64 %k.010, 1
  %cmp = icmp slt i64 %inc52, %x
