...
static void
emitVEDeclareSimdFunction(const FunctionDecl *FD, llvm::Function *Fn,
                           const llvm::APSInt &VLENVal,
                           ArrayRef<ParamAttrTy> ParamAttrs,
                           OMPDeclareSimdDeclAttr::BranchStateTy State) {
  struct ISADataTy {
  ...
  uint64_t VectorWidth = VLENVal > 0 ? VLENVal.getZExtValue() : 256;
  llvm::SmallVector<char, 2> Masked;
  switch (State) {
  case OMPDeclareSimdDeclAttr::BS_Undefined:
    Masked.push_back('N');
  ...
  for (char Mask : Masked) {
    ...
    llvm::raw_svector_ostream Out(Buffer);
    char VECode = 'v';
    Out << "_ZGV" << VECode << Mask << VectorWidth;
    for (const ParamAttrTy &ParamAttr : ParamAttrs) {
      switch (ParamAttr.Kind){
      case LinearWithVarStride:
        Out << 's' << ParamAttr.StrideOrArg;
...
void CGOpenMPRuntime::emitDeclareSimdFunction(const FunctionDecl *FD,
  ...
  while (FD) {
    for (const auto *Attr : FD->specific_attrs<OMPDeclareSimdDeclAttr>()) {
      ...
      } else if (CGM.getTriple().getArch() == llvm::Triple::ve) {
        emitVEDeclareSimdFunction(FD, Fn, VLENVal, ParamAttrs, State);
      }
    }
