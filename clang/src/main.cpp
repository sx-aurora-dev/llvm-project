...
public:
    void EndSourceFileAction() override {
        ...
        //llvm::raw_fd_ostream outFile("output.txt", error_code, llvm::sys::fs::F_Append);
    ...
    CreateASTConsumer(clang::CompilerInstance &CI, clang::StringRef) final
    {
        ...
        // TargetCode holds all necessary information about source locations of target regions
        // to extract that code
        Code = new TargetCode(TargetCodeRewriter);
        return std::unique_ptr<clang::ASTConsumer>(new TargetRegionTransformer(*Code,
                                                                               TargetCodeRewriter));
    }
};
...
int main(int argc, const char **argv)
{
    ...
    return tool.run(clang::tooling::newFrontendActionFactory<SourceTransformAction>().get());
}
